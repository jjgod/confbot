...
class PositionError(ConfigObjError):
    """
    A keyword has come after a section.
...
##            print >> sys.stderr, sline, mat
            if mat is not None:
                # is a keyword value
                if this_section.sections != []:
                    # we already have a section
                    # so shouldn't have any keywords
                    self._handle_error(
                        "Keyword in wrong place at line %s.",
                        PositionError, infile, cur_index)
                    continue
